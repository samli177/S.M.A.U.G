...
void LegGoto(double x,double y, int z, int servospeed, int side, int servo1, int servo2, int servo3)
{
	...
	*/
	...
	alpha = acos((femur*femur-tibia*tibia+d*d)/((double)2*femur*d))-asin(fabs(z)/d);
...
{
	double a = (x + centerToFrontLegsX)*cos((double)(-3*3.1415/4))-(y-centerToFrontLegsY)*sin((double)(-3*3.1415/4));
	double b = (x + centerToFrontLegsX)*sin((double)(-3*3.1415/4))+(y-centerToFrontLegsY)*cos((double)(-3*3.1415/4));
